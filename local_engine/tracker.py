...
class UltimateTracker:
    ...
    def detect(self, frame_bgr: np.ndarray) -> TrackResult:
        # Downscale for MediaPipe — it internally resizes to 256x256 anyway.
        # Landmarks are normalized 0-1 so they map correctly to any resolution.
        h_orig, w_orig = frame_bgr.shape[:2]
        small = cv2.resize(frame_bgr, (w_orig // 2, h_orig // 2))
        ...
        # Pose at half frequency — body moves slowly, hands need every frame
        self._frame_count += 1
        if self._frame_count % 2 == 0 or self._cached_pose is None:
            self._cached_pose = self._process_pose(
                self.pose_tracker.detect_for_video(mp_image, t))

        return TrackResult(hands=hands, pose=self._cached_pose)
