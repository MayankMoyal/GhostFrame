...
Optimized pipeline per frame (parallel):
1. [Parallel A] Run MediaPipe tracker on the raw frame.
2. [Parallel B] Run RVM to blend user foreground over background.
3. [Wait] Collect results from both.
4. Render 'Front' props (Z >= 0) onto the composited frame.
5. Render 'Behind' props onto next frame's background (1-frame latency).
...
def camera_loop(args):
    ...
    Optimized pipeline: MediaPipe tracking and RVM matting run in parallel
    via ThreadPoolExecutor since they are independent operations.
    Behind-layer props use previous frame's tracking data (1-frame latency,
    imperceptible at 30+ fps) to avoid blocking the parallel pipeline.
    """
    ...
    # Parallel executor for MediaPipe + RVM
    executor = ThreadPoolExecutor(max_workers=2, thread_name_prefix="ghost")

    # Cached background: only re-resize when the source image changes
    _cached_bg_src_id = None
    _cached_bg_resized = None

    # Previous frame's tracking for behind-prop rendering (1-frame latency)
    prev_track_result = None
    ...
    while True:
        ...
        # Prepare background (cached — only resize when source changes)
        with bg_lock:
            bg_src = bg_image_holder["image"]
            bg_src_id = id(bg_src)
        if bg_src_id != _cached_bg_src_id or _cached_bg_resized is None:
            _cached_bg_resized = cv2.resize(bg_src, (w, h))
            _cached_bg_src_id = bg_src_id

        # Render 'Behind' props onto a copy of background using PREVIOUS
        # frame's tracking (1-frame latency, imperceptible at 30+ fps)
        with props_lock:
        ...
        current_bg = _cached_bg_resized.copy()
        if back_props and prev_track_result is not None:
            for pdata in back_props:
                current_bg = render_prop(
                    current_bg, pdata, prev_track_result, mirror, w, h, args.debug)

        # --- PARALLEL: Run MediaPipe tracking + RVM matting concurrently ---
        track_future = executor.submit(tracker.detect, raw)
        rvm_future = executor.submit(remover.remove, display_frame, current_bg)

        track_result = track_future.result()
        comp = rvm_future.result()

        # Save tracking for next frame's behind-props
        prev_track_result = track_result

        # Render 'Front' props onto the composited image (needs current tracking)
        for pdata in front_props:
            comp = render_prop(comp, pdata, track_result, mirror, w, h, args.debug)

        # Build and broadcast tracking data for OBS overlay
